...
Control.ProgressBar = Class.create({
    ...
    setProgress: function(value){
        this.progress = value;
        this.draw();
        $('progress_percent').update(value + '%'); //More page-specific hacode.
    ...
    poll: function (url, interval, ajaxOptions){
        ...
        ajaxOptions.onSuccess = success.wrap(function (callOriginal, request) {
	     var upload = request.responseText.evalJSON();
             var progressPercent = Math.floor((upload.received / upload.size) * 100);
		...
		{ this.poller.stop(); }
            if(finished)
            {
	      		$('progress_bar').hide();  //Gah, page-specific code, bad! Fix by re-styling progress_bar instead.
	      		$('progress_percent').hide(); 
	      		$('progress').update('Done, attempting OGG conversion...');
            }
